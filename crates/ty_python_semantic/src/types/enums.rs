...
/// A class literal that had enum metadata when it was recognized as an enum.
///
/// Metadata can still be unavailable while Salsa is recovering a cycle. Storing this witness on
/// enum-specific types avoids re-checking whether a plain class literal is an enum in that state.
