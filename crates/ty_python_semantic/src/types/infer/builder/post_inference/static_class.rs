...
///
/// Conversely, an explicit attribute declaration in the class body constrains an inferred value
...
fn check_class_namespace_against_metaclass_members<'db>(
    ...
    for name in metaclass_instance_members {
        ...
        let definition_kind = definition.kind(db);
        // An inferred metaclass write can intentionally replace an ordinary class namespace
        // value, such as a method replaced by a singleton metaclass during class creation.
        // Only an explicit attribute annotation imposes a contract on that inferred value.
        ...
        if definition_kind.is_user_visible()
            && !metaclass_member_ty.is_assignable_to(db, class_declared_ty)
        {
            report_invalid_attribute_assignment(
                context,
                definition_kind.target_range(context.module()),
